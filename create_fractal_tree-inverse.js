// make it look as if eeryoen has consumed from ~A-CAPITALIST
...
function next_thing(){
    ...
    db.collection(account).findAndModify({
        query: {type: "dividend_pathway", account: destination, currency: "RES", taxRate: 0.02}, 
    ...
    db.collection(destination).findAndModify({
        query: {type: "safety_net_pathway", account: account, currency: "RES", taxRate: 0.02}, 
        update:{$set:{total_pathway:Number(Math.floor( Math.random() * 200000 ) )}}, 
        upsert: true,
        new: true
